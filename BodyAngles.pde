...
void setup() {
 ...
 //Open the serial port, I think we might want to save this for bluetooth stuff, not sure tho. 
...
void draw() {  
  ...
  if (userList.size() > 0) {
    ...
    if ( kinect.isTrackingSkeleton(userId)) {
     ...
     println("shoulderAngle: " + shoulderAngle); 
     sendOsc(elbowAngle, shoulderAngle);                  
    }
